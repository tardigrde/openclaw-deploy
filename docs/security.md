...
1. **API Credentials**
   ...
   - See secrets(secrets.md) for the full secrets model (two-file layout, SOPS encryption, how secrets reach the VPS)
...
3. **Tailscale VPN (Recommended)**
   ...
   **What this does not protect against:**
   ...
   Tailscale is installed automatically on first boot when `TF_VAR_enable_tailscale=true` is set. See [Firewall Rules](index.md#firewall-rules) in the README for the step-by-step setup.
...
6. **State Files**
   ...
   - Default backend is local; GCS remote state (with versioning and encryption at rest) is recommended for teams — see remote state(remote-state.md)
...
- Automated security patching (you must update OpenClaw manually — see versions(versions.md))
...
- Secrets encryption at rest — SOPS/age encryption for `.env` (see secrets(secrets.md))
