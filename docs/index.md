...
7. *(Optional)* **Remote Terraform state** — the default is local (no setup needed). For GCS or other remote backends, see [Remote State](remote-state.md).
...
Required: `HCLOUD_TOKEN`, `TF_VAR_ssh_key_fingerprint` (from [Hetzner Console](https://console.hetzner.cloud/) → Security → SSH Keys). See [Secrets Reference](secrets.md) for the full variable reference and Tailscale-specific notes.
...
Required: `OPENCLAW_GATEWAY_TOKEN`, `ANTHROPIC_API_KEY` (or leave empty for subscription auth), `TELEGRAM_BOT_TOKEN`. See [Secrets Reference](secrets.md) for the full variable reference.
...
Plain `.env` works for local use. SOPS is **required** for the [GitOps auto-deploy workflow](gitops-auto-deploy.md). See [SOPS](secrets.md#sops-encryption-optional-for-local-required-for-ci) for the full workflow.
...
Default backend is local (no setup needed). See [Remote State](remote-state.md) for GCS setup, migration from local state, and CI authentication.
...
See **[Configuration](openclaw-config.md)** for:
...
See [CI/CD Setup](cicd.md) for required GitHub Variables, Secrets, and approval gate setup.

## Advanced Topics

| Topic | Doc |
| ----- | --- |
| Skills (ClawHub) | [Skills](skills.md) |
| Headless browser | [Headless Browser](headless-browser.md) |
| Multi-agent setup | [Agents](agents.md) |
| Workspace Git sync | [Workspace Git Sync](workspace-git-sync.md) |
| Morning weather report | [Weather Report](addons/weather-report.md) |
| CI/CD setup | [CI/CD Setup](cicd.md) |
| GitOps auto-deploy | [GitOps Auto-Deploy](gitops-auto-deploy.md) |
| Security hardening | [Security Hardening](security-hardening.md) |
| Version management | [Versions](versions.md) |
| Remote state backend | [Remote State](remote-state.md) |
| Secrets reference | [Secrets Reference](secrets.md) |

## Security

See [Security Policy](security.md) for the full security policy and threat model.
